...
const AdminCourseStructurePage: React.FC = () => {
  ...
  const [courseStructure, setCourseStructure] =
    useState<ExtendedCourseStructure | null>(null);
  ...
  const [isCompletedUsersModalOpen, setIsCompletedUsersModalOpen] =
    useState(false);
  ...
  const [selectedSectionVideoUrl, setSelectedSectionVideoUrl] =
    useState<string>("");
  ...
  const [aiSubchapters, setAISubchapters] = useState<
  ...
  const fetchCourseData = async () => {
    try {
      ...
      const structureData = await courseStructureService.getCourseStructure(
        courseId || ""
      );
      ...
      if (structureData) {
        ...
        if (structureData.chapters) {
          const chaptersState = structureData.chapters.reduce<{
            [key: string]: boolean;
          }>((acc, chapter) => {
          ...
          const subchaptersState = structureData.chapters.reduce<{
            [key: string]: boolean;
          }>((acc, chapter) => {
  ...
  const handleGenerateAIChapter = async () => {
    ...
    const hasValidSubchapter = aiSubchapters.some(
      (sub) => sub.title.trim() !== ""
    );
    ...
    try {
      ...
      const validSubchapters = aiSubchapters.filter(
        (sub) => sub.title.trim() !== ""
      );
      ...
      const savedChapter = await courseStructureService.addChapter(
        courseId || "",
        newChapter
      );
  ...
  const handleAddChapter = async (newChapter: any) => {
    ...
    try {
      ...
      const formattedChapter: ExtendedChapter = {
        ...
        subchapters: newChapter.subchapters.map(
          (subchapter: any, subIndex: number) => ({
            id: subchapter.id,
            title: subchapter.title,
            description: subchapter.description,
            sections: subchapter.sections.map(
      ...
      const savedChapter = await courseStructureService.addChapter(
        courseId,
        formattedChapter as Chapter
      );

      // Fetch the updated course structure
      const updatedStructure = await courseStructureService.getCourseStructure(
        courseId
      );
  ...
  const handleCourseCompletion = async (data: {
  ...
  const saveVideoUrl = async (videoUrl: string) => {
    ...
    try {
      const response = await fetch(
        ...
        }/api/courses/${courseId}/structure/section/${selectedSectionId}/video`,
        {
          method: "PATCH",
          headers: {
      ...
      <div className="container mx-auto px-4 py-8">
        {loading ? (
          ...
          <>
                      ...
                      <Card key={chapterId || chapterIndex} className="mb-6">
                        <CardHeader
                          className={`cursor-pointer ${
                            expandedChapters[chapterId] ? "border-b" : ""
                          }`}
                          ...
                          <CardContent className="pt-4">
                            {chapter.subchapters.map(
                              (subchapter, subchapterIndex) => {
                                const subchapterId = getItemId(subchapter);
                                const subchapterQuizzes = getSubchapterQuizzes(
                                  subchapter.title
                                );
                                return (
                                  <Card
                    ...
                    <CardContent className="flex flex-col items-center justify-center">
                      <div className="text-center">
                        ...
                        <p className="text-gray-500 mb-4">
                          This course doesn't have a structure defined
                        </p>
                        <Button
                          onClick={() => setIsAIChapterModalOpen(true)}
              ...
              <TabsContent value="videos">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Manage video content for course sections. Videos will be
                      displayed in the learning module when students reach the
                  ...
                  <CardContent>
                    {courseStructure?.chapters?.length ? (
                      ...
                      <div className="text-center py-8">
                        <p className="text-gray-500 mb-4">
                          No course structure available. Create a course
                          structure first to add videos.
                        </p>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    {chapterToDelete?.title}"? This action cannot be undone and
                    will remove all subchapters, sections, and associated
                    quizzes.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
            ...
            <Dialog
              open={isAIChapterModalOpen}
              onOpenChange={setIsAIChapterModalOpen}
            >
              <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>Generate Chapter with AI</DialogTitle>
                  <DialogDescription>
                    Provide chapter and subchapter details. The AI will create
                    sections and content based on this information.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="chapterDescription">
                  ...
                  <div className="mt-4">
                    ...
                    {aiSubchapters.map((subchapter, index) => (
                      <div
                        key={index}
                        className="grid gap-2 mb-4 p-3 border rounded-md"
                      >
                        <div className="flex items-center justify-between">
                          <Label htmlFor={`subchapterTitle-${index}`}>
                            Subchapter {index + 1} Title*
                          </Label>
                        ...
                        />
                        <Label htmlFor={`subchapterDescription-${index}`}>
                ...
                <DialogFooter>
                  <Button
                    variant="outline"
                    onClick={() => setIsAIChapterModalOpen(false)}
                  >
                    Cancel
                  </Button>
                  <Button
                    ...
                    disabled={
                      aiGenerating ||
                      !aiChapterTitle.trim() ||
                      !aiSubchapters.some((sub) => sub.title.trim() !== "")
                    }
                  >
                    {aiGenerating ? (
                      <>
