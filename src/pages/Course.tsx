...
const Course: React.FC = () => {
  ...
  const starcBooks = [/* keep your books exactly as you wrote them */];
    ...
    const fetchCourses = async () => {
      ...
      try {
        ...
        const formattedCourses: Course[] = courses.map((course: ICourse) => ({
          ...
          level: course.level,
          category: course.category === "AI Generated" ? "Development" : course.category,
  ...
  const applyFilters = (courses: Course[]) => {
    ...
    switch (sortOption) {
      ...
      case "Highest Rated":
        ...
        break;
      case "Newest":
        filtered = filtered.slice().sort((a, b) => (b._id || "").localeCompare(a._id || ""));
        break;
      default:
  ...
  const displayCourses = mode === "supervised" ? coursesData : starcBooks as unknown as Course[];
      ...
      <div className="flex justify-center mb-20">
        {loading ? (
          ...
          <section className="flex relative justify-center py-8 mb-16 xl:w-[90%] flex-col md:flex-row">
            {/* sidebar, no changes */}
            <section className="hidden md:block sticky h-[80vh] top-[90px] lg:w-[25%] overflow-y-auto px-2" style={{ scrollbarWidth: "thin" }}>
              <div className="w-full">
                <div className="max-w-c mx-auto px-4 sm:px-6 lg:px-10">
                  <h2 className="text-2xl font-mont font-bold mb-4 py-2 text-gray-800 text-center">Categories</h2>
                    ...
                    {categories.map((category, index) => (
                      <li
                        ...
                        className={`py-4 px-4 lg:text-[10px] xl:text-sm cursor-pointer w-[95%] border-b-[0.1px] ${
                          index === activeIndex ? "text-[#8A63FF] font-semibold" : "text-gray-800"
                        }`}
                      >
                        {category}
                      </li>
            ...
            <div className="flex flex-wrap justify-center md:justify-start w-full md:w-[80%] gap-5 md:gap-6">
              {filteredCourses.length > 0 ? (
                ...
                <div className="w-full text-center py-8 text-gray-500">No courses found in this category.</div>
              )}
            </div>
          </section>
        )}
...
const FilterSection: React.FC<FilterSectionProps> = ({
  ...
  const [activeButton, setActiveButton] = useState(initialActiveButton || "supervised");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col">
          <div className="flex items-center gap-4 flex-col">
            <div className="flex items-center bg-white-500 p-1 rounded-full" style={{ boxShadow: "0px 0px 22.4px 0px #00000040" }}>
              <button
                onClick={() => handleToggle("supervised")}
                className={`px-4 py-2 rounded-full font-semibold ${activeButton === "supervised" ? "bg-[#8A63FF] text-white" : "bg-white text-gray-600"}`}
              >
                Supervised
              </button>
              <button
                onClick={() => handleToggle("unsupervised")}
                className={`px-4 py-2 rounded-full font-semibold ${activeButton === "unsupervised" ? "bg-[#8A63FF] text-white" : "bg-white text-[#8A63FF]"}`}
              >
                Unsupervised
              </button>
            </div>
            <span className="text-sm text-[#8A63FF] text-center">{description}</span>
          </div>
        </div>
