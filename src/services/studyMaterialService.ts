...
export const getStudyMaterials = async (
  ...
  try {
    ...
    const response = await axios.get(`${API_ENDPOINT}/course/${courseId}`, {
      headers,
    });
...
export const getAllStudyMaterials = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${ADMIN_API_ENDPOINT}/course/${courseId}/all`,
      { headers }
    );
    return response.data.data || [];
  } catch (error) {
...
export const createStudyMaterial = async (
  studyMaterialData: Partial<IStudyMaterial>
): Promise<IStudyMaterial> => {
  try {
...
export const reorderStudyMaterials = async (
  courseId: string,
  materialIds: string[]
): Promise<IStudyMaterial[]> => {
  try {
